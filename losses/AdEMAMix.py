"""
from:
https://github.com/nanowell/AdEMAMix-Optimizer-Pytorch/tree/main
"""
...
class AdEMAMix(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999, 0.9999), eps=1e-8, weight_decay=0.0, alpha=5.0, T_alpha_beta3=None):
        if not 0.0 <= lr:
    ...
    def _update_adamemix(
        self, params, grads, exp_avgs, exp_avg_sqs, exp_avg_slow, state_steps, beta1, beta2, beta3, alpha, T_alpha_beta3, lr, weight_decay, eps
    ):
        for i, param in enumerate(params):
            ...
            if T_alpha_beta3 is not None:
                ...
                beta3_t = min(
                    math.exp(math.log(beta1) * math.log(beta3) / ((1 - step / T_alpha_beta3) * math.log(beta3) + (step / T_alpha_beta3) * math.log(beta1))),
                    beta3,
                )
            else:
                alpha_t = alpha

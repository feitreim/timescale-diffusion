...
class TimescaleLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        if normalization == 'exp':
            self.normalization = torch.tensor([2 ** ((i + 1) / self.bin_size) - 2 ** (i / self.bin_size) for i in range(latent_size)])
        ...
        assert self.offset >= 0, 'LOSS INITIALIZATION ERROR: Offset must be non-negative'

    def timescale_loss_l1(self, latents: torch.Tensor, time_steps: torch.Tensor) -> torch.Tensor:
        """
        ...
        time_offset_matrix = torch.abs(time_offset_matrix - time_offset_matrix.t()) + self.offset
        # Calculate bin start indices
        bin_start_indices = torch.ceil(torch.log2(time_offset_matrix) * self.bin_size).to(torch.int32)  # BxB TODO: Switch to match pyramid levels?
        bin_start_indices = torch.clamp(bin_start_indices, min=0, max=(latents.size()[1]))
            ...
            for j in range(distances.size()[0]):
                if bin_start_indices[i, j] < latents.size()[1]:
                    if latents.dim() == 2:
                        ...
                        distance /= (self.normalization[bin_start_indices[i, j] : latents.size(1)] / self.normalization[latents.size(1) - 1]).to(
                            distance.device
                        )
                    elif latents.dim() == 4:
                        distance = torch.nn.functional.l1_loss(
                        ...
                        distance /= (self.normalization[bin_start_indices[i, j] : latents.size(1)].view(-1, 1, 1) / self.normalization[latents.size(1) - 1]).to(
                            distance.device
                        )
                    else:
    ...
    def timescale_loss_l2(self, latents: torch.Tensor, time_steps: torch.Tensor) -> torch.Tensor:
        """
    ...
    def timescale_loss_contrastive(self, latents: torch.Tensor, time_steps: torch.Tensor) -> torch.Tensor:
        """
        ...
        for i in range(B):
            for j in range(B):
                if i == j or bin_start_indices[i, j] >= L:
                    ...
                    denominators_ij = torch.nn.functional.normalize(denominators_ij, dim=1)
                    denominator_ij = torch.sum(denominators_ij * denominators_ij[i]) - 1.0

...
class TimeBias(nn.Module):
    def __init__(self, t_dim, dim_size):
        super().__init__()
        self.bias = nn.Parameter(torch.rand((1, 1, dim_size, dim_size)), requires_grad=True)
        self.scalar = nn.Parameter(torch.randn((1, t_dim * 2, dim_size * dim_size)), requires_grad=True)
    ...
    def forward(self, x, t):
        ...
        bias = torch.mul(self.bias, scalar)
        biased = torch.cat([x, bias], dim=1)
        return biased
...
class StructuralDecoder(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, n_res_layers, res_h_dim, stacks):
        ...
        self.first_time_embed = TimeBias(10, 32)
        self.first_up_conv = nn.Sequential(
            nn.ConvTranspose2d(in_dim + 1, h_dim, kernel_size=kernel - 1, stride=stride - 1, padding=1),
            nn.ReLU(),
            ResidualStack(h_dim, h_dim, res_h_dim, n_res_layers),
            nn.ConvTranspose2d(h_dim, h_dim // 2, kernel_size=kernel, stride=stride, padding=1),
            nn.ReLU(),
        )
        self.second_time_embed = TimeBias(10, 64)
        ...
        self.third_time_embed = TimeBias(10, 128)
        self.third_up_conv = nn.Sequential(
            nn.ConvTranspose2d(h_dim + 1, h_dim // 2, kernel_size=kernel, stride=stride, padding=1),
            nn.ReLU(),
            ResidualStack(h_dim // 2, h_dim // 2, res_h_dim, n_res_layers),
